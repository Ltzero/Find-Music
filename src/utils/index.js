...
export const findIndexByKey = (arry, attr, key) => arry.findIndex((item) => {
  if (item[`${attr}`] === key) {
    return true;
  }
});
...
export const objectArrayConcatAndRemoveDuplicate = (array1, array2) => {
  const conCatArray = array1.concat(array2); // 两个数组对象合并
  const newArray = []; // 盛放去重后数据的新数组
  ...
  switch (this.state.playMode) {
    ...
    case 'random_cycle':
      ...
      if (this.state.myList[random].playUrl === '') {
        const { id } = this.state.myList[random];
    ...
    case 'list_order':
      ...
      if (index === PA1) {
        ...
        store.dispatch(action);
        // 前面已经没有了
      } else if (this.state.myList[PA2].playUrl === '') {
          const { id } = this.state.myList[PA2];
          this.getMusicUrl(id).then((data) => {
            const value = {
              index: PA2,
              playUrl: data,
    ...
    case 'list_cycle':
      ...
      if (index === PA1) {
        if (this.state.myList[PA3].playUrl === '') {
          const { id } = this.state.myList[PA3];
          this.getMusicUrl(id).then((data) => {
            const value = {
              index: PA3,
              playUrl: data,
