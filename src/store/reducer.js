// 引入actiontypes
import {
  GET_SONG_DETAILS, GET_LRC, COMPUTED_LRC, ADD_TO_MY_LIST, OPEN_MUSIC_LIST, CLOSE_MUSIC_LIST, SWITCH_MUSIC_PLAYER, CHANGE_PLAYER_MUSIC, SET_PLAYER_PROGRESS, REMOVE_FROM_MY_LIST, CHANGE_MUSIC_ORDER, CHANGE_PLAYER_MODE, SEARCH_SINGLE_MUSIC, GET_SONG_LIST_DETAILS, ADD_SONG_LIST_TO_MY_LIST, ADD_SONG_PLAY_URL, SET_SEARCH_KEY, SEARCH_SONG_LISTS, GET_ALBUM_DETAILS, SEARCH_ALBUM,
} from './actionTypes';
...
export const reducer = (state = defaultState, action) => {
  ...
  switch (action.type) {
    ...
    case COMPUTED_LRC:
      ...
      return newState;
    case ADD_TO_MY_LIST:
      const isRepeat = newState.myList.some(item => item.id === action.value.id);
    ...
    case REMOVE_FROM_MY_LIST:
      ...
      if (action.value === newState.audio.id) {
        ...
        newState.myList = newState.myList.filter(item => item.id !== action.value);
